...
|---------|---------|-------|
| Nuxt dev server | `bun run dev` | Serves on http://localhost:3000. Works without Convex backend for frontend-only work. |
| Convex backend | `npx convex dev` | Requires a Convex deployment + env vars (`NUXT_PUBLIC_CONVEX_URL`, `CONVEX_SITE_URL`). Not needed for pure frontend/styling tasks. |

### Quick reference

- **Lint**: `bun eslint app/ convex/` (the `.agents/` skill scripts have style issues that are not part of the app)
- **Test**: `bun run test` (Vitest with edge-runtime, tests in `app/**/*.test.ts` and `convex/**/*.test.ts`)
- **Typecheck**: `bun run typecheck` (runs `nuxt typecheck` via `vue-tsc`)
- **Dev server**: `bun run dev`
- **Prepare types** (needed before lint/typecheck on a fresh install): `bun run nuxt prepare`

### Gotchas

- ESLint config imports from `.nuxt/eslint.config.mjs` which is generated by `nuxt prepare`. If lint fails with a missing module error, run `bun run nuxt prepare` first.
- The `better-convex-nuxt` module logs a warning about missing `siteUrl` when `NUXT_PUBLIC_CONVEX_URL` is empty — this is harmless for frontend-only development.
- The Nuxt app uses `ssr: false` for most routes (SPA mode) except `/updates/**` and `/_og/**`. The dev server still works fine for all routes.
- `.env` is gitignored. Copy `.env.example` to `.env` and fill in at minimum `SITE_URL=http://localhost:3000` for the dev server to start cleanly.
