...
2. **Sign-in** — Use a provisioned admin (see [admin-auth-setup.md](./admin-auth-setup.md)). Expect redirect to `/admin` (or the `redirect` target).
...
5. **Public site** — `/`, `/posts`, and post detail pages still load without auth.
